        ...
        if settings.REDIS_ENABLED:
            if redis_client:
                try:
                    ...
                    components.append(ComponentStatus(name="cache", status="healthy", details={"type": "redis"}))
                except Exception as redis_ex:
                    logger.error(f"Redis health check failed: {str(redis_ex)}")
                    components.append(
                        ComponentStatus(
                            name="cache", status="unhealthy", details={"error": str(redis_ex), "type": "redis"}
                        )
                    )
                    all_healthy = False
            else:
                components.append(
                    ComponentStatus(
                        name="cache",
                        status="unhealthy",
                        details={"error": "Redis client not initialized", "type": "redis"},
                    )
                )
                all_healthy = False
        else:
            components.append(
                ComponentStatus(name="cache", status="disabled", details={"type": "redis", "enabled": False})
            )
    except Exception as e:
        logger.error(f"Redis health check failed: {str(e)}")
        components.append(
            ComponentStatus(name="cache", status="unhealthy", details={"error": str(e), "type": "redis"})
        )
        all_healthy = False

    # Check Kafka
    try:
        # Log the actual value of KAFKA_ENABLED for debugging
        logger.info(f"KAFKA_ENABLED setting value: {settings.KAFKA_ENABLED}, type: {type(settings.KAFKA_ENABLED)}")
        logger.info(f"KAFKA_BOOTSTRAP_SERVERS: {settings.KAFKA_BOOTSTRAP_SERVERS}")

        # Check if Kafka is enabled (handle both string and boolean values)
        kafka_enabled = settings.KAFKA_ENABLED
        if isinstance(kafka_enabled, str):
            kafka_enabled = kafka_enabled.lower() != "false"

        if kafka_enabled:
            if kafka_producer:
                try:
                    # Check if already connected
                    if kafka_producer.connected():
                        components.append(
                            ComponentStatus(
                                name="message_broker",
                                status="healthy",
            ...
            # Kafka is disabled, so mark as disabled (not unhealthy)
            logger.info("Kafka is explicitly disabled in settings")
            components.append(
                ComponentStatus(name="message_broker", status="disabled", details={"type": "kafka", "enabled": False})
            )
    except Exception as e:
        ...
        components.append(
            ComponentStatus(
                name="message_broker",
                status="unhealthy",
                details={"error": str(e), "type": "kafka", "bootstrap_servers": settings.KAFKA_BOOTSTRAP_SERVERS},
            )
        )
        all_healthy = False
